...
void test()
{
    ...
    require_close(calculator.evaluate("2 + 3 * 4"), 14.0);
    require_close(calculator.evaluate("20 % 6"), 2.0);
    require_close(calculator.evaluate("2 ^ 3 ^ 2"), 512.0);
    require_close(calculator.evaluate("5!"), 120.0);
    require_close(calculator.evaluate("[2 + 3] * {4 + 1}"), 25.0);
    require_close(calculator.evaluate("set a 5!"), 120.0);
    require_close(calculator.evaluate("a / (3!)"), 20.0);
    require_close(calculator.evaluate("-2 ^ 2"), -4.0);
}
...
void demo()
{
    ...
    {
        std::print("{} = {}\n", example, calculator.evaluate(example));
    }
}
}
...
void test()
{
    task_12_19::require_close(parse("2 + 3 * 4"), 14.0);
    task_12_19::require_close(parse("20 % 6"), 2.0);
    task_12_19::require_close(parse("2 ^ 3 ^ 2"), 512.0);
    task_12_19::require_close(parse("5!"), 120.0);
    task_12_19::require_close(parse("[2 + 3] * {4 + 1}"), 25.0);
    task_12_19::require_close(parse("-2 ^ 2"), -4.0);
}
